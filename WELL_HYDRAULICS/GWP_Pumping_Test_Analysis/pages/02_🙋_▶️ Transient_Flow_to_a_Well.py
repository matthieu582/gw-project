...
with st.expander(":green[**Show/Hide the second assessment**]"):
    ...
    stb.single_choice(":blue[**If the estimated transmissivity $T$ is too low, how will the predicted drawdown compare to the true drawdown?**]",
                  ...
                  3,success='CORRECT! ... the low transmissivity will require a higher gradient to accommodate flow through the aquifer surrounding the well screen, so predicted drawdown will be larger than true drawdown near the well. This results in more of the pumped volume being extracted from storage near the well bore, thus less of the volume will be drawn from storage further away resulting in less drawdown at larger distances from the well. It may not be possible to observe this on the graph for every combination of parameter values entered in this module because the r distance on the graph is limited to 1000 m and the distance where drawdown transitions from overestimation to underestimation may be further than 1000 m from the well. If you are unable to see the transition, then decreasing the values input for time and/or distance might bring the transition zone into view.', error='This is partially correct because the low transmissivity will require a higher gradient to accommodate flow through the aquifer surrounding the well screen, so predicted drawdown will be larger than true drawdown near the well. This results in more of the pumped volume being extracted from storage near the well bore, thus less of the volume will be drawn from storage further away resulting in less drawdown at larger distances from the well. At some location depending on the combination of parameter values the drawdown may be the same for both cases. It may not be possible to observe this on the graph for every combination of parameter values entered in this module because the r distance is limited to 1000 m and the distance where drawdown transitions from overestimation to underestimation may be further away than 1000 m. If that is the case, decreasing the values input for time and/or distance might bring the transition zone into view. Feel free to answer again.')
...
st.markdown('''
            ...
            The next part of this module demonstrates how to estimate aquifer parameter values using drawdown data measured near a pumping well. You can move to the next section using either the menu on the left side or the navigation buttons at the bottom of this page.
            '''
)
"---"
