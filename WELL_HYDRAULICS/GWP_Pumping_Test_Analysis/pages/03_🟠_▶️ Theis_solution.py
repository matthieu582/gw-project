...
st.markdown(""" 
            The Theis (1935) solution was developed to calculate drawdown due to pumping a confined aquifer. Please note: The assumption of a uniform potentiometric level before the start of pumping, as shown in the subsequent figure, is not required. Indeed, natural systems typically have some hydraulic gradient. This background level may even change over the duration of a test. Therefore, the analyst must identify the changes in water levels caused only by pumping (i.e., drawdowns).
...
with st.expander('**Click here for more information** about the underlying theory of the :red[**Theis solution**]'):
    ...
    Another module that provides all materials and guidance for **manual Theis type-curve matching** is available at [https://theis-manual.streamlit.app/](https://theis-manual.streamlit.app/).
    """
    )
